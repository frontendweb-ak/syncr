 ...
 * APP.PAGE_SIZE_MAX so a client can never request an unbounded
...
export function getPaginationParams(c: Context<AppContext>): PaginationParams {
  ...
  const rawPageSize = Number.parseInt(
    c.req.query("pageSize") ?? String(APP.PAGE_SIZE_DEFAULT),
    10,
  );
  ...
  const pageSize =
    Number.isFinite(rawPageSize) && rawPageSize > 0
      ? Math.min(rawPageSize, APP.PAGE_SIZE_MAX)
      : APP.PAGE_SIZE_DEFAULT;
